...
from src.models.run_rq2_nested_cross_validation import (
...
RFC_PATH = (
    ...
    / "data"
    / "features"
    / "rfc_bench_with_affective_features.csv"
)
...
def main() -> None:
    """Run the full RFC external robustness evaluation."""
    ...
    seed_metrics.to_csv(
        TABLE_DIR
        / "rq2_rfc_external_seed_metrics.csv",
        index=False,
    )

    summary.to_csv(
        TABLE_DIR
        / "rq2_rfc_external_summary.csv",
        index=False,
    )
    ...
    predictions.to_csv(
        PREDICTION_DIR
        / "rq2_rfc_external_predictions.csv",
        index=False,
    )

    print("\nRFC external evaluation complete.")
    print(
        "Training datasets:",
        len(DATASETS),
    )
