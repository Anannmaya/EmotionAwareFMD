...
Each model variant is tuned separately using the same predefined search grid
and the same inner folds. The outer test fold is never used for tuning.
"""
...
def build_feature_matrices(
    ...
    x_evaluation = hstack(
        [
            x_evaluation_text,
            csr_matrix(x_evaluation_affect),
        ],
        format="csr",
    )
...
def parameter_sort_key(
    ...
    return (-ngram_end, min_df, -c_value)
...
def tune_model_on_outer_training_data(
    ...
    best_selection_key: tuple[float, float, int, int, float] | None = None
    ...
    for row in tuning_rows:
        row["selected"] = (
            ...
            and row["min_df"] == int(best_parameters["min_df"])
            and row["C"] == float(best_parameters["C"])
        )
...
def main() -> None:
    ...
    combined_fold_metrics.to_csv(
        TABLE_DIR / "rq2_nested_fold_metrics.csv",
        index=False,
    )
    combined_repetition_metrics.to_csv(
        TABLE_DIR / "rq2_nested_repetition_metrics.csv",
        index=False,
    )
    statistical_summary.to_csv(
        TABLE_DIR / "rq2_nested_statistical_comparison.csv",
        index=False,
    )
    combined_selected_parameters.to_csv(
        TABLE_DIR / "rq2_nested_selected_hyperparameters.csv",
        index=False,
    )
    combined_tuning_scores.to_csv(
        TABLE_DIR / "rq2_nested_inner_tuning_scores.csv",
        index=False,
    )
    combined_predictions.to_csv(
        PREDICTION_DIR / "rq2_nested_oof_predictions.csv",
        index=False,
    )

    print("\nNested RQ2 experiment complete.")
