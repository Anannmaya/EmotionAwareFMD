...
INPUT_PATH = Path(
    "data/features/liar2_finance_with_affective_features.csv"
)

DESCRIPTIVE_OUTPUT = Path(
    "outputs/tables/liar2_affective_descriptive_statistics.csv"
)

TEST_OUTPUT = Path(
    "outputs/tables/liar2_affective_paired_tests.csv"
)
...
def holm_adjust(p_values: list[float]) -> np.ndarray:
    """Apply Holm's correction for multiple hypothesis tests."""
...
def paired_rank_biserial(differences: pd.Series) -> float:
    ...
    Positive values mean the feature is generally higher
    in misleading statements than in genuine statements.
    """
    nonzero_differences = differences[differences != 0].to_numpy()
...
def validate_dataset(dataframe: pd.DataFrame) -> None:
    ...
    if len(dataframe) != 2048:
        raise ValueError(
            f"Expected 2,048 rows, found {len(dataframe):,}."
        )

    if dataframe[FEATURE_COLUMNS].isna().any().any():
        raise ValueError("Missing affective feature values detected.")

    if set(dataframe["binary_label_name"].unique()) != {
        "genuine",
        "misleading",
    }:
        raise ValueError("Unexpected label values detected.")

    pair_check = (
        dataframe.groupby("matched_pair_id")
        .agg(
            rows=("binary_label_name", "size"),
            labels=("binary_label_name", "nunique"),
        )
    )

    if not (pair_check["rows"] == 2).all():
        raise ValueError("Every matched pair must contain two rows.")

    if not (pair_check["labels"] == 2).all():
        raise ValueError(
            "Every matched pair must contain both labels."
        )
    ...
    for feature in FEATURE_COLUMNS:
        for label in ["genuine", "misleading"]:
            values = dataframe.loc[
                dataframe["binary_label_name"] == label,
                feature,
            ]
...
def run_paired_tests(dataframe: pd.DataFrame) -> pd.DataFrame:
    results = []

    for feature in FEATURE_COLUMNS:
        paired_values = dataframe.pivot(
            index="matched_pair_id",
            columns="binary_label_name",
            values=feature,
        )
...
def main() -> None:
    ...
    print("RQ1 analysis completed.")
    print(f"Matched pairs: {dataframe['matched_pair_id'].nunique():,}")
